...
export default function CreateGoal() {
	...
	const {createSpace} = usePolkadotContext();
	...
	async function createGoalInEVM(spaceid = null) {
		...
		try {
			enqueueSnackbar(`Creating Goal in Moonbase network...`, {variant: "info"});
		...
		enqueueSnackbar("Successfully created Goal in moonbase network", {variant: "success"});
	...
	async function createGoal() {
		var CreateGoalBTN = document.getElementById("CreateGoalBTN");
		CreateGoalBTN.disabled = true;
		await createSpace(createGoalInEVM);
	}
						...
						<div className="flex gap-8" style={{color: "black"}}>
							<div style={{boxShadow: "#907979 0 0 10px 0px"}} className="bg-white rounded-lg flex flex-col p-2  pb-2 w-48 pb-0">
								<h6
									...
									className="border-2 hover:bg-[#d1d5db] hover:cursor-pointer bg-white flex hover:bg-gray-200 items-center p-2 rounded-lg w-full"
								>
