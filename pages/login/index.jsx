...
export default  function Login() {
  ...
  const fetchDataStatus = async () => {
    if (
       window.localStorage.getItem("login-type") == "Metamask" && window.localStorage.getItem("login-type2") =="Polkadot"
    ) {
  ...
  useEffect(() => {
    if (!isServer()) {
      setInterval(() => {
        if ( window.localStorage.getItem("login-type") == "Metamask"&& window.localStorage.getItem("login-type2") == "Polkadot") {
  ...
  async function onClickConnectPolkadot() {
    ...
    await polkadot.web3Enable('AmazingGreen');
2
    ...
    <>
      ...
      <div className={`${styles.container} flex items-center flex-col gap-8`}>
        <div className={`${styles.title} flex flex-col`}>
          ...
          <p className="mt-4 text-trunks">Please connect to your Metamask or Polkadot.js wallet in order to login.</p>
          ...
          <p className="text-trunks">Polkadot.js: Moonbase alpha(Default)</p>
        </div>
